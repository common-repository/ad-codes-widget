...
/*
The __autoload function for all Ad Codes widget classes.
...
*/
if (!function_exists ("ws_widget__ad_codes_classes")) /* Already exists? */
	{
		function ws_widget__ad_codes_classes ($class = FALSE) /* Build dynamic __autoload function. */
			{
				...
				/**/
				if (strpos ($class, "c_ws_widget__ad_codes_") === 0 && strpos ($class, "c_ws_widget__ad_codes_pro_") === false)
					{
						$c = (!isset ($c)) ? dirname (dirname (__FILE__)) . "/classes" : $c; /* Configures location of classes. */
						$c_class_dirs = (!isset ($c_class_dirs)) ? array_merge (array ($c), _ws_widget__ad_codes_classes_scan_dirs_r ($c)) : $c_class_dirs;
						/**/
						$class = str_replace ("_", "-", str_replace ("c_ws_widget__ad_codes_", "", $class));
						/**/
		...
		function _ws_widget__ad_codes_classes_scan_dirs_r ($starting_dir = FALSE)
			{
					...
					if (is_dir ($starting_dir)) /* Does this directory exist? */
						foreach (scandir ($starting_dir) as $dir) /* Scan this directory. */
							if ($dir !== "." && $dir !== ".." && is_dir ($dir = $starting_dir . "/" . $dir))
								$dirs = array_merge ($dirs, array ($dir), _ws_widget__ad_codes_classes_scan_dirs_r ($dir));
				/**/
				return $dirs; /* Return array of all directories. */
			}
		/**/
		spl_autoload_register ("ws_widget__ad_codes_classes"); /* Register __autoload. */
	}
?>